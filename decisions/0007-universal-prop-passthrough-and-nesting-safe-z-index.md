...
Accepted — implemented in full (2026-09-14). `Textarea`, `Badge`, `Checkbox`, `Input` all extended to the passthrough pattern; `className` merged on all four plus `Button`; `Select`-in-`Dialog` z-index fixed. Pre-merge code review then found and fixed a real regression this branch introduced (`Drawer` outranking `Dialog`/`AlertDialog`) plus three narrower correctness gaps of the same shape (`Badge`'s `role`, `Checkbox`'s `children`/`asChild`, `Textarea`/`Input`'s `children`). See Verified below.
...
- The z-index fix's scope stayed narrow on purpose: only `Select`↔`Dialog`, the pair with an actual failing test. `Toast`/`Tooltip` keep their own tiers — either nested inside `Dialog`/`Drawer` (or inside each other) could have the identical bug, unconfirmed either way. Flagged in `docs/backlog.md`, not fixed speculatively. (`Drawer`'s own instance of this exact bug — introduced by this branch, not pre-existing — was found by code review and fixed; see Verified.)
...
### Code review found and fixed two more real issues (2026-09-14, same day)

A multi-angle review of this branch, before merge, found one real regression this branch itself introduced and three narrower correctness gaps in the same "type admits a prop the component silently mishandles" shape as the defect this whole ADR exists to fix. All four confirmed by reproducing red, then green — not fixed speculatively.

- **Regression: `Drawer` started outranking `Dialog`/`AlertDialog`.** Before this branch, `.dialog__content` and `.drawer__content` were both explicitly `--z-modal` (400) — a tie, correctly resolved by DOM/mount order. Dropping `Dialog`'s z-index to `auto` while leaving `Drawer`'s explicit broke that tie the other way: an explicit z-index on a sibling always outranks `auto`, regardless of mount order, so `Drawer` started always winning — the exact bug class this ADR fixes, reintroduced for a different pair, with a real path to it (an `AlertDialog` triggered from inside an already-open `Drawer`, e.g. a delete confirmation). Fixed by dropping `Drawer.css`'s `.drawer__overlay`/`.drawer__content` to `auto` too, matching `Dialog`. `Dialog.nesting.test.tsx` (`Drawer` opened first, `AlertDialog` opened afterward via a real click — not both mounted in one initial render, which trips an unrelated real Radix/`aria-hidden` bookkeeping edge case) reproduces Ajar's exact failure shape (`drawer__overlay intercepts pointer events`) before the fix, passes after.
- **`Badge`, `Checkbox`, `Textarea` (and `Input`, same shape, not flagged but fixed for consistency) each newly admitted a prop they silently mishandle.** Widening to the real native/Radix attributes type is correct, but each component also *computes or fixes* one of those attributes itself rather than truly passing it through: `Badge` always sets its own `role` (a deliberate a11y decision) after spreading `...rest`, so a consumer's `role` was silently overwritten; `Checkbox` always renders a fixed check/indeterminate icon as `RadixCheckbox.Root`'s real children, so a consumer's `children` was silently discarded and `asChild` would have merged onto that fixed markup instead of a real element; `Textarea`/`Input` are controlled via `value`/`onChange`, so a consumer's `children` landing in `...rest` hits a controlled `<textarea>` (a React dev warning, a possible hydration mismatch) or a void `<input>` (a hard React runtime error, confirmed by the first version of `Input.ref.test.tsx`'s own contract test crashing at mount). Each now explicitly `Omit`s the offending key(s), with a `@ts-expect-error` contract test asserting the rejection — the directive itself is the assertion; if the `Omit` list is ever widened back, typecheck fails. Every such test constructs the offending JSX without rendering it (mounting is a separate, already-covered concern; `Input`'s case would crash React outright if actually mounted).
- `npm run validate` green throughout the fixes: 214/214 tests (up from 209), 0 lint/architecture/contrast violations. `docs/components/{badge,checkbox,textarea,input}.md` regenerated; `--z-modal` is now also fully unreferenced (`Drawer` dropped it too) alongside `--z-dropdown`/`--z-overlay`.
