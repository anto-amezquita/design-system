...
- Make one brand's expression a component's default. The default is the functional behaviour every consumer can live with; a signature animation is an opt-in prop (`motion="expressive"` on Button), and the wrapper that turns it on lives in the consuming app, not here — ADR [`0016`](decisions/0016-functional-button-default-expressive-opt-in.md) extends 0001's token split to behaviour. An animation library belongs behind a dynamic `import()` on that opt-in path, so it is never fetched or executed by an app that doesn't use it.
...
- Treat a draft in `decisions/proposed/` as a decision, or commit one. They're gitignored, unreviewed, and mostly noise by design — accepting one means rewriting it as a numbered ADR by hand (see ADR [`0009`](decisions/0009-living-memory-adr-proposals.md)).
