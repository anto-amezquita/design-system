...
- **DTCG-compliant tokens** (`$value`/`$type`) — three layers: global primitives → semantic tokens → component tokens, resolved through [Style Dictionary](https://styledictionary.com); 638 tokens total — 112 global primitives and 526 semantic + component tokens, each resolved across all four theme axes
