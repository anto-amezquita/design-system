...
Everything missing is *distribution*, not foundation. The system is governed; it is not yet legible to an agent that has never seen it.

| Gap | Benchmark context |
|---|---|
| `llms.txt` + markdown twin per component | 10/37 field, 14/20 leaders |
| `AGENTS.md` at repo root | 15/20 leaders |
| Single agent-facing validate entrypoint | The leaders' "validation loops turn guidelines into gates" pattern |
| Agent skill served from `/.well-known/skills/` | 18/20 leaders |
| shadcn-spec registry (`npx shadcn add <url>`) | 1/37 field — a category of one |
...
- **A hosted MCP server.** An agent skill delivers most of the same value at a fraction of the maintenance surface for a solo maintainer. Revisit only if a second consumer needs it.
