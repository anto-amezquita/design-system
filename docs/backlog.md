...
No published npm version contains any of this. Every consumer — Ajar included, which independently hit and worked around several of these exact bugs (its own `decisions/0030`) — is blocked until a release ships. Next step: `npm run changeset` (patch — every change here is additive: wider prop types, a previously-silent `className` now working, a CSS fix; no existing consumer's working code stops working), then the existing release workflow (merge to `main` → bot opens "Version Packages" PR → merging that publishes). Not new engineering.

Status: code complete on branch `fix/0007-prop-passthrough-and-z-index`, `npm run validate` green (209/209). Needs: changeset, PR review, merge, then the release PR merged separately (a deliberate second checkpoint — that merge is what actually publishes, and this package also ships to portfolio).
