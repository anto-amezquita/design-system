...
| `test` | Every story as a browser test + `unit` project contracts (ADR [`0006`](../decisions/0006-add-layered-automated-testing.md)) + `scripts` project tooling tests (ADR [`0009`](../decisions/0009-living-memory-adr-proposals.md)) | `vitest run` — `storybook`/`unit` in real Chromium via Playwright, `scripts` in Node |
...
- Cross-brand/mode consistency: currently `portfolio-light`/`portfolio-dark` have story-level Chromatic coverage; `base-light`/`base-dark` do not yet (same gap `bold` had before it was removed — noted in `docs/backlog.md`'s token-architecture backlog). Don't claim 4-mode visual coverage until this is closed.
...
- [ ] `npm run tokens:audit` if component tokens were added — check they resolve differently from their referent in at least one mode, or are literal; a pass-through/chain-skip token is a collapse candidate, not new precedent (see the token-architecture backlog in `docs/backlog.md`)
