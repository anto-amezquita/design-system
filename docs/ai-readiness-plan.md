...
Next action:    Task 2.2 — add a fabrication check to tokens:lint: fail on
                any var(--…) in component CSS not present in the resolved
                token set, and fail on any var(--token, fallback)
                two-argument form.
Blocked on:     nothing
```
