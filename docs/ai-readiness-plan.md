...
Next action:    Task 1.3a — include all global primitives in
                token-reference.json (currently only color.*; 83 primitives
                invisible), then emit a meta breakdown (primitiveCount /
                semanticCount / componentCount / total).
Blocked on:     nothing
```
