...
Next action:    Task 1.4 — wire the CI staleness check: rebuild, git diff
                --exit-code, fail if anything changed. Already wired into
                npm run tokens; extend chromatic.yml's existing "Check
                generated files are in sync" step to also cover tokens.json,
                docs/components/, llms.txt, and llms-full.txt (currently only
                checks dependency-graph.json/token-reference.json/
                component-registry.json/styles/brands/).
Blocked on:     nothing
```
