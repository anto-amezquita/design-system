...
.claude/           — Claude Code config for this repo (settings.json: the
                     SessionEnd/SessionStart hook pair, ADR 0009). Distinct from
                     hooks/ above, which is React hooks.
...
decisions/         — ADRs, one file per significant architectural choice
  proposed/             — harvested session drafts, NOT decisions (gitignored,
                          never auto-committed; see decisions/proposed/README.md)
